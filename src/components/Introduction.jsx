  ...
  return (
    <div id='bod' className=" h-full px-15  items-center flex justify-between bg-[#11142C]">
      <div id='ctf' onMouseMove={(e)=>{move(e)}} onMouseLeave={(el)=>{
        simple(el)
      }} className="pb-18 mt-20 pt-20 w-[50%] h-full ">
        ...
        <div id='cts' className="w-[88%] gap-3  flex justify-between mt-9">
          <button id='download'className="px-14 rounded-2xl gap-3  whitespace-nowrap py-3 bg-[#997FE1] flex justify-center hover:scale-[1.02] transition-[0.2s] items-center">
            {" "}
            <ArrowDownToLine size={18} /> Download Resume
          </button>
          <a href="#contact">

            <button id='contact-btn' className="px-8 gap-3 py-2 flex whitespace-nowrap justify-center border-1 border-[#997FE1] rounded-2xl text-white items-center hover:bg-[#997FE1] text-gray-950 transition-[0.2s]">
            {" "}
            <Mail size={18} /> Contact me
          </button>
          </a>
        </div>
        <div id='icons' className="flex mt-7 gap-8 px-2 text-gray-300">
          ...
          <a  href="https://www.facebook.com/share/1ARV6HsuZz/" className=" hover:bg-[#11172cb0] px-3 py-3 rounded-full "> <Facebook /></a>
            <div id='down' ref={next} className="absolute left-[50%] " >
              <a href="#about"> <ChevronDown size={30} /></a> 
            </div>
          </div>
      </div>
      <div id='imag'className="w-[30%] [perspective:1000px] h-full mt-8 flex justify-end items-center  ">
        <div
          ref={boxRef}
        id='penatgon'
          className="h-[500px] bg-white rounded-b-[160px] overflow-hidden"
        >
